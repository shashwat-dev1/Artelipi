...
import { getFollowing } from '@/lib/firebase/follow';
...
export default function FollowingPage() {
    ...
    const loadFollowing = async () => {
        ...
        try {
            ...
            // Get following IDs
            const followingIds = await getFollowing(profileUser.uid);
            ...
            const validFollowing = followingUsers.filter((u): u is User => u !== null);
            ...
            <div className="min-h-screen bg-white py-8">
                <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="mb-8">
                        ...
                        <p className="text-gray-600 mt-1">{following.length} following</p>
                    </div>
                    ...
                    {following.length > 0 ? (
                                    ...
                                    <div className="flex items-center justify-between">
                                        <Link href={`/${followedUser.username}`} className="flex items-center gap-4 flex-1">
                                            ...
                                            <div className="flex-1">
                                                ...
                                                {followedUser.bio && (
                                                    <p className="text-gray-600 text-sm mt-1 line-clamp-2">{followedUser.bio}</p>
                                                )}
                                                <div className="flex items-center gap-4 text-xs text-gray-500 mt-2">
                                                    <span>{followedUser.followerCount || 0} followers</span>
                                                    <span>·</span>
                                                    <span>{followedUser.totalArticles || 0} articles</span>
                                                </div>
                                            </div>
                                        </Link>
                                        <FollowButton
                        ...
                        <div className="text-center py-16 border border-gray-200 rounded-lg">
                            ...
                            <p className="text-gray-500">When {user.name} follows people, they'll appear here</p>
                        </div>
                    )}
                </div>
            </div>
